...
shell = None  # Global reference to GUI instance
...
def quarantine_origin(ip, country):
    ...
    def auto_block():
        ...
        if shell:
            shell.block_list.addItem(f"{ip} ({country})")
            shell.update_threat_table()
...
class ASIGUI(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ASI Protection Shell vΩ.5")
