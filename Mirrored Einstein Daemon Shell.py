...
class EinsteinShellGUI:
    ...
    def build_gui(self):
        ...
        self.tab_control.add(self.sync_tab, text='Sync Bus')
        ...
        self.sync_status = tk.Label(self.sync_tab, text="Syncing nodes...", font=("Arial", 14))
    ...
    def update_gui(self, drift, svg, glyph_key):
        ...
        self.sync_status.config(text=f"Nodes synced: {random.randint(3, 9)}")
        timestamp = time.strftime('%H:%M:%S')
        self.log_box.insert(tk.END, f"[{timestamp}] Glyph: {glyph_key}\n")
        self.log_box.see(tk.END)
        self.update_formula_panel(drift)

    def update_formula_panel(self, drift):
