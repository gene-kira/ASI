...
class CodexGUI(tk.Tk):
    def __init__(self,swarm):
        super().__init__()
        self.title("Codex + Devourer Dashboard")
        self.swarm=swarm; self.e=swarm.nodes[0].e
        self.errs=[]; self.muts=[]; self.cpu=[]; self.mem=[]; self.step=0; self.last=time.perf_counter()
        self.fs_log=tk.Text(self,height=10,width=60); self.fs_log.pack(side="top",fill="x")
        self.net_log=tk.Text(self,height=5,width=60); self.net_log.pack(side="top",fill="x")
    ...
    def draw(self):
        ...
        for i,n in enumerate(self.swarm.nodes):
            ...
            self.ax_swarm.text(i+0.5,0.5,f"{n.id[:6]}\nerr={avg:.2f}",
