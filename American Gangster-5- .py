...
def ingest(data, origin, persona):
    ...
    registry[ts] = event
    if gut(event) or encrypted(event):
        event["gut"] = True
        event["lineage"]["purge"] = "gut/encrypted"
        purge(data)
...
# 🖥️ GUI
def gui():
    root = tk.Tk(); root.title("🧙 Codex Sentinel — Living Shell"); root.geometry("800x600"); root.configure(bg="#1e1e2f")
