...
class MagicBoxGUI:
    def __init__(self, root, swarm_path=None):
        ...
        self.selected_style = tk.StringVar(value=self.settings.get("style", "curious"))
        self.status_text = tk.StringVar(value="🧠 Status: Initializing")
        self.scan_interval = tk.IntVar(value=self.settings.get("interval", 60))
        global save_path
        save_path = self.settings.get("save_path", None)
        ...
        ttk.Button(self.control_frame, text="🔧 Manual Sync Path", command=self.manual_sync_path).grid(row=1, column=4, padx=5)
    ...
    def update_scan_label(self, value):
        ...
        self.freq_value_label.config(text=f"{interval} sec")
        self.settings["interval"] = interval
        save_settings(self.settings)

    def on_style_change(self, event):
        self.settings["style"] = self.selected_style.get()
        save_settings(self.settings)

    def manual_sync_path(self):
        path = filedialog.askdirectory()
        if path:
            self.swarm_path = path
            self.event_list.insert(tk.END, f"🔧 Manual sync path set: {path}")
