...
# + heavy predictive engine (temporal memory, drift, operator modeling, multi-node forecasting)
# ============================================================
...
class PredictiveEngine:
    """
    Heavy predictive brain:
    - Temporal memory (hour buckets)
    - Operator activity modeling
    - Drift & anomaly scoring
    - Rolling risk score [0..100]
    - Multi-node signal hooks (swarm can integrate later)
    """
    def __init__(self, config):
        self.lock = threading.Lock()

        # Temporal frequency: hour -> event_type -> count
        self.temporal_events = {}  # {hour: {event: count}}
        # Anomaly history: timestamps and scores
        self.anomaly_history = []  # list of (timestamp, event_type, score)
        # Operator activity: hour -> "active" weight
        self.operator_activity = {}  # {hour: weight}

        # Rolling risk in 0..100
    ...
    def _get_hour_bucket(self, ts=None):
        ...
        return lt.tm_hour  # 0..23

    def observe_event(self, event_type, source, is_anomaly=False):
        """
        Called for every system/line event.
        Updates temporal memory, operator model, and risk.
        Returns current risk after update.
        """
        now = time.time()
        ...
        with self.lock:
            ...
            # Update operator activity (we treat any event as "user/system alive")
            self.operator_activity[hour] = self.operator_activity.get(hour, 0) + 1

            # Compute novelty / rarity
            ...
            # If it's not anomaly, we actually decrease risk slightly (stability)
            if not is_anomaly:
                delta = -min(delta, 5.0)
    ...
    def _compute_rarity(self, event_type, hour):
        """
        Higher score for events that rarely occur in this hour.
        """
        hour_map = self.temporal_events.get(hour, {})
        total = sum(hour_map.values())
        if total == 0:
            # No history yet ⇒ unknown hour ⇒ modest rarity
            return 5.0
    ...
    def _time_of_day_weight(self, hour):
        """
        Weight anomalies differently depending on operator activity.
        If hour is usually quiet but something happens, weight > 1.
        If hour is busy, anomalies still matter but slightly less.
        """
        total_activity = sum(self.operator_activity.values()) or 1
        this_activity = self.operator_activity.get(hour, 0)
        activity_ratio = this_activity / total_activity  # 0..1

        # If almost no activity historically at this time: anomalies amplified
...
class LineComputer:
    ...
    def process(self, event, source="line"):
        ...
        # Predictive observation happens outside lock
...
class SwarmNode:
    ...
    def sync(self, other_node):
        with self.lock, other_node.lock:
            ...
            # Basic risk blending across nodes
...
class SwarmManager:
    ...
    def _run(self, interval):
        while self.running:
            ...
            # Also let nodes "see" current global risk
