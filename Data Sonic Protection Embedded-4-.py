...
# === IMPORTS ===
import os, time, threading, socket, psutil, datetime, json, wave, pyaudio, numpy as np
import tkinter as tk
from tkinter import ttk, Canvas, Text
from threading import Lock

# === CONFIG ===
RULES = {'backdoor': 3, 'no_mac_ip': 30, 'personal': 86400, 'fake_telemetry': 30}
...
def destroy(kind, payload):
    ...
    with LOCK: LOG.append((kind, payload, d, wave_data))
...
class Shell:
    def __init__(self, root):
        self.root = root; root.title("MagicBox ASI Guardian"); root.geometry("1200x900")
        ...
        self.canvas = Canvas(root, width=600, height=150, bg='black'); self.canvas.pack()
        self.polarity = Canvas(root, width=600, height=150, bg='white'); self.polarity.pack()
        self.sound_btn = ttk.Button(root, text="Sound: ON", command=self.toggle_sound); self.sound_btn.pack()
        self.timeline = Text(root, height=10, bg='lightyellow'); self.timeline.pack(fill='both', padx=10, pady=10)
        self.timers = {}; self.panel = ttk.LabelFrame(root, text="Timer Control"); self.panel.pack(fill='x', padx=10)
        for i, k in enumerate(RULES):
            ttk.Label(self.panel, text=f"{k} (sec):").grid(row=i, column=0)
            v = tk.StringVar(value=str(RULES[k]))
            ttk.Entry(self.panel, textvariable=v, width=10).grid(row=i, column=1)
            self.timers[k] = v
