...
from ad_signature_tracker import load_signatures
...
def inject_js(driver, stealth=False):
    ...
    selectors = [f'[id="{s["id"]}"]' for s in sigs if s["id"]] + [f'[class="{s["class"]}"]' for s in sigs if s["class"]]
    selector_str = ", ".join(selectors) if selectors else '[id*="ad"], [class*="ad"], iframe[src*="ads"]'
    ...
    driver.execute_script(f"""
        window.addEventListener('load', () => {{
            setTimeout(() => {{
                ...
                document.querySelectorAll('{selector_str}').forEach(e => {{
