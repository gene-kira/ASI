...
def guess_b4b_paths() -> Dict[str, List[str]]:
    # Safe, symbolic targets for preloading (no file modifications, just awareness)
    ...
    return {
        ...
        "game_assets": [
            # Steam common (game content)
            expand_vars(f"{programfilesx86}\\Steam\\steamapps\\common\\Back 4 Blood"),
            # Epic default install hint (varies by user choice)
        ...
        "steam_artifacts": [
            expand_vars(f"{steam_base}\\steamapps\\appmanifest_1293860.acf"),  # Back 4 Blood appid hint
...
class PredictiveLearner:
    ...
    def predict(self, sysd: Dict[str, Any], cands: List[str], top_k: int = 8):
        ...
        for n in range(1,min(self.max_n,len(seq))+1):
            ...
            for c in cands: scores[c]+=counts.get((prefix,c),0)*n
        ...
        while self.running:
            try:
                ...
                if preds and risk["risk_low"]:
                    top, conf = preds[0]
                    targets = self.preloader._targets_for(top)
...
class ScannerGUI:
    def __init__(self, root: tk.Tk, inventory: InventorySnapshot, policy: PolicyEngine,
        ...
        # Start removed (auto-start); Stop remains
