...
def get_cpu_state():
    ...
    return entropy_to_state(cpu / 100)

def get_mem_state():
    mem = psutil.virtual_memory().percent
    return entropy_to_state(mem / 100)
...
# Symbolic Memory Pattern from Live Metrics
def generate_memory_pattern():
    ...
    for value in metrics:
        entropy = min(value / 100, 1.0)
        pattern.append(entropy_to_state(entropy).state)
    return pattern
