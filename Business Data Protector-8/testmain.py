    ...
    def loop():
        ...
        while True:
            try:
                ...
                # Inject test threat and codex entries every 10 seconds
                counter += 1
                app.threat_matrix_panel.after(0, lambda: app.threat_matrix_panel.log_threat(f"🔥 Test threat #{counter}"))
                brain.codex.append({
                    "timestamp": time.strftime("%H:%M:%S"),
                    "label": f"Test Codex Entry #{counter}",
                    "source": "Diagnostic"
                })
                app.codex_dashboard_panel.after(0, lambda: app.codex_dashboard_panel.load_codex(brain.export_codex()))

                # Simulate traffic and defense logic
