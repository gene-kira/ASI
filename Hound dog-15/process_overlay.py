import psutil
import random
from glyphs import pulse_glyph, spawn_glyph
from thinking_overlay import update_thoughts
...
def animate_process_overlay(canvas, scale=1.0, origin_x=0, origin_y=0):
    ...
    for pid, dot in process_dots.items():
        ...
        if dot["threat"]:
            pulse_glyph(canvas, dot["x"], dot["y"], scale, origin_x, origin_y)
...
def handle_process_click(event, canvas, scale, origin_x, origin_y, app):
    mx = canvas.canvasx(event.x)
    my = canvas.canvasy(event.y)
    for pid, dot in process_dots.items():
        x = dot["x"] * scale + origin_x
        y = dot["y"] * scale + origin_y
        if abs(mx - x) < 10 and abs(my - y) < 10 and dot["threat"]:
            try:
                psutil.Process(pid).terminate()
                canvas.create_text(x, y-15, text="💀 Quarantined", fill="red", font=("Helvetica", 8), tags="process")
                spawn_glyph(app.glyphs, "red")
                update_thoughts(f"Quarantined {dot['name']} (PID {pid})")
