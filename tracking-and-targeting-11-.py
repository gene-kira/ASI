...
pygame.display.set_caption("Autonomous Tracking + Manual Lock")
...
def pick_auto_target(tracks,protected):
    best=None; bests=-1e9
    for t in tracks:
...
locked_id=None; click_indicator_pos=None; click_indicator_timer=0
LOCK_THRESHOLD=30
...
try:
    while True:
            ...
            if event.type==pygame.QUIT: raise KeyboardInterrupt
                ...
                if event.button==1:  # left click
                    ...
                    for t in tracker.tracks:
                        ...
                        cx_disp=int(cx*WIDTH/640)
                        cy_disp=int(cy*HEIGHT/360)
                    ...
                    if closest_id is not None and min_dist<=LOCK_THRESHOLD:
                        ...
                        logging.info(f"Manually locked target ID {locked_id}")
                elif event.button==3:  # right click unlock
                    locked_id=None
                    click_indicator_pos=None
                    click_indicator_timer=0
                    logging.info("Manual lock released")
        ...
        if locked_id is not None:
            target_track=next((t for t in tracks if t.id==locked_id),None)
        else:
            target_id=pick_auto_target(tracks,protected_display)
        ...
        if target_track is not None:
            cx,cy=target_track.get_position()
            cx_disp=int(cx*WIDTH/640); cy_disp=int(cy*HEIGHT/360)
            pygame.draw.circle(screen,GREEN,(cx_disp,cy_disp),8,2)
            if locked_id==target_track.id:
                screen.blit(font.render(f"LOCKED ID {locked_id}",True,GREEN),(cx_disp+10,cy_disp-30))
        ...
        # HUD
        fps=clock.get_fps()
        status=f"FPS: {fps:.1f} | Tracks: {len(tracks)} | Dets: {len(detections)} | Target: {locked_id if locked_id else 'Auto'}"
