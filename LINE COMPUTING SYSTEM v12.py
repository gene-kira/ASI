# ============================================================
# LINE COMPUTING SYSTEM v10
# Autonomous, predictive, adaptive, defensive, personalized guardian
#
# Features:
# - Engine + GUI + watchdog + swarm + system backplane
# - Profiling + per-process whitelist/blacklist + policy engine
# - Profile metadata + strict mode + hybrid ALLOW-folding
# - Heavy predictive engine:
#     * Temporal memory (hour buckets)
#     * Multi-dimensional risk scoring (event type, CPU, behavior deviation)
#     * Operator modeling (active/idle, time-of-day patterns)
#     * Drift detection
# - Process behavioral fingerprints (per-process baselines)
# - Cross-node risk/forecast sharing (swarm)
# - Auto-start + auto-profiling (one-week learning window ready)
# ============================================================
...
# Optional psutil for deeper system hooks and per-process stats
try:
...
def load_settings():
    ...
    # Seeded baseline for a typical Windows box (editable).
    return {
        ...
        # Actions: "allow" | "log" | "alert" | "kill" | "ignore"
        "process_policies": {
        ...
        "predictive": {
            ...
            "operator_idle_seconds": 600  # 10 min; after this, treat as idle
        },
...
# -----------------------------
# PROCESS BEHAVIOR MODEL (fingerprints)
# -----------------------------
class ProcessBehaviorModel:
    """
    Per-process behavioral fingerprints (CPU, lifetime, start frequency).
    Only active if psutil is available.
    """
    def __init__(self):
        self.lock = threading.Lock()
        self.stats = {}  # name_upper -> fingerprint dict
    ...
    def compute_deviation_score(self, pid, name):
        ...
        try:
            ...
            # Very rough heuristic: high CPU is suspicious
            if cpu > 50:
...
class PredictiveEngine:
    """
    Multi-dimensional predictive model:
    - Temporal frequency (hour buckets)
    - Anomaly rarity
    - Operator activity and idle windows
    - Behavior deviation weight from ProcessBehaviorModel (when available)
    """
    ...
    def observe_event(self, event_type, source, is_anomaly=False, behavior_score=0.0):
        ...
        with self.lock:
            ...
            self.last_operator_activity_ts = now  # any event indicates activity
            ...
            if self.is_operator_idle():
                delta *= 1.3  # events during idle time more suspicious
...
fold_manager = None  # created in main()
...
class ProcessPolicyEngine:
    ...
    def apply_action(self, pid: str, name: str, action: str, event_type: str, source: str,
        ...
        if psutil and pid not in (None, "?", ""):
            ...
            if behavior_score > 0:
                log.add("INFO", f"{SYMBOL_FP} Behavior deviation for {name} (pid={pid}) score={behavior_score:.1f}")
...
class SystemEventCollector:
    ...
    def _check_processes(self):
        ...
        # Behavioral model: observe starts/ends
