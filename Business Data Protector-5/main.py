...
import sys, subprocess, threading, time, psutil, requests
...
required_libs = ["tkinter", "psutil", "requests", "hidpi-tk"]
...
def geo_block_ip(app, ip):
    country = get_country_from_ip(ip)
    if blocked_countries.get(country, False):
        block_ip_temporary(ip, duration=3600)
        trigger_deception_overlay("GeoShield")
        inject_persona("Decoy")
        log_codex(f"🛡️ Geo-blocked IP: {ip} → {country}")
        app.threat_matrix_panel.after(0, lambda: app.threat_matrix_panel.log_threat(f"🚨 Blocked {ip} from {country}"))
    ...
    def daemon_loop():
        ...
        while True:
            try:
                ...
                app.codex_dashboard_panel.after(0, lambda: app.codex_dashboard_panel.load_codex(app.memory))
