...
Self-evolving, secure, predictive scaffold with:
- Telemetry, input monitoring, persona inference
- Predictor (Markov + extended k-gram + bandit + calibration)
- Planner, Policy engine, AutoLoader with speculative read-ahead
- CryptoManager for per-machine AES-GCM
- Logger, MemoryManager, SelfImprover
- GUI with tabs: Predictor, Borg Mesh, Logs, Telemetry, Policy, Planner, AutoLoader, Capability
- 🔎 Anomaly detection hooks + adaptive exploration
- 🕸️ Borg Mesh overlay (mesh, roles, guardian, comms)
- 🔺 Windows elevation fix (flag-aware, no relaunch loops)
"""
...
def ensure_admin_once() -> bool:
    """
    Returns True if already elevated or elevation completed.
    Returns False if elevation was successfully launched and parent should exit.
    Continues without elevation on failure (no crash loops).
    """
    if platform.system() != "Windows":
        return True
...
# -------------------------------------------------------------------
# Telemetry collector (real probes + input + activity + personas)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Predictor (ensemble + personas + extended k-gram + bandit + calibration)
# -------------------------------------------------------------------
...
class Orchestrator:
    ...
    def _loop(self, interval_sec: float):
        ...
        while not self.stop_flag:
            ...
            anomaly_info = self.anomaly.observe(snap)
            exploratory = bool(anomaly_info.get("active"))
...
# -------------------------------------------------------------------
# GUI Manager — tabs: Predictor, Borg Mesh, Logs, Telemetry, Policy, Planner, AutoLoader, Capability
# -------------------------------------------------------------------
...
def build_system(project_root: str = ".", iface_cidr: Optional[str] = None) -> Dict[str, Any]:
    ...
    orchestrator = Orchestrator(
        logger, telemetry, predictor, planner, policy_engine, autoloader, improver, anomaly,
...
def main(cli_mode: Optional[bool] = None, iface_cidr: Optional[str] = None):
    # Elevation gate — only once, only on Windows
    ok = ensure_admin_once()
    if not ok:
        return  # parent exits after launching elevated child
    ...
    if not cli_mode and TK_AVAILABLE:
        gui = GUIManager(
            predictor, telemetry, logger, memory, mesh,
            orchestrator=orchestrator,
            capability=capability,
            policy_engine=policy_engine
        )
...
if __name__ == "__main__":
    print("Starting Fortune Teller (self-improving, encrypted, highly predictive, Borg Mesh overlay, elevation-safe)...")
    if "--cli" in sys.argv:
