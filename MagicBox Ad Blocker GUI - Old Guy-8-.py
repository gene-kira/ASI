...
SWARM = "https://your-swarm-node.net/vote"
PORTS = [80, 443]
...
def gui():
    ...
    logbox = tk.Text(root, height=10, width=90, bg="#1f1f1f", fg="#0f0", font=("Courier", 10)); logbox.pack(pady=10)
    fwbox = tk.Text(root, height=15, width=90, bg="#1f1f1f", fg="#fc0", font=("Courier", 10)); fwbox.pack(pady=10)

    def log(msg): logbox.insert(tk.END, msg + "\n"); logbox.see(tk.END)
    globals()["log"] = log

    def refresh():
        fwbox.delete(1.0, tk.END)
        for x in load()["block"]:
            fwbox.insert(tk.END, f"{x['domain']} ({x['ip']}) → Ports: {x['ports']} | Source: {x['source']} | Entropy: {x['entropy']} | {x['symbol']}\n")
