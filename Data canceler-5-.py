import subprocess, sys, random, time, socket, threading
...
class EchoNull(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("EchoNull v4: Mutation-Aware Sentinel Shell")
    ...
    def symbolic_purge(self):
        glyphs = ["ΔΔΔ", "drift fracture", "echo bleed"]
        recent = " ".join(self.lineage[-5:])
        if any(g in recent for g in glyphs):
            self.log.append("🧨 Symbolic Purge Triggered")
            self.lineage.append("Purged rogue feedback via glyph match")
            self.polarity = 0
            self.radar.setText("Polarity Radar: [0]")
    ...
    def broadcast_state(self):
        try:
            msg = f"{self.polarity}|{hash(tuple(self.lineage[-5:]))}"
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            sock.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_TTL, 2)
            sock.sendto(msg.encode(), ("224.0.0.1", 9999))
        except Exception as e:
            self.log.append(f"[ERROR] Broadcast failed: {e}")
    ...
    def listen_mesh(self):
        try:
            ...
            while True:
                ...
                polarity, remote_hash = data.decode().split("|")
                if int(polarity) > 200:
                    self.status.append(f"⚠️ Remote node drift: {polarity}")
        except Exception as e:
            self.status.append(f"[ERROR] Mesh listener failed: {e}")
