...
# 🔄 Feed Map Canvas
feed_canvas = tk.Canvas(root, bg=BG_COLOR, width=620, height=150, highlightthickness=0)
feed_canvas.pack(pady=10)
...
for label, (x, y) in nodes.items():
    feed_canvas.create_oval(x-8, y-8, x+8, y+8, fill=BTN_COLOR)
    feed_canvas.create_text(x, y+15, text=label, fill=TEXT_COLOR, font=("Segoe UI", 10))

def animate_feeds():
    feed_canvas.delete("line")
    for source in ["System", "Web", "Swarm"]:
        x1, y1 = nodes[source]
        x2, y2 = nodes["ASI"]
        feed_canvas.create_line(x1, y1, x2, y2, fill="#00ffcc", width=2, tags="line")
    root.after(1000, animate_feeds)
