...
# 🧠 Sentinel Protocol – Zero Trust ASI Guardian
...
# 🌌 Real-Time Threat Monitor (Simulated)
def threat_monitor():
    while True:
        entity = f"rogue_{random.randint(100,999)}"
        behavior = random.choice(["loop", "scan", "inject"])
...
tk.Label(root, textvariable=output, wraplength=550, fg="lightgreen", bg="#1e1e2f", font=("Courier", 10)).pack(pady=5)

memory_output = tk.StringVar()
tk.Label(root, text="Symbolic Memory Log:", font=("Helvetica", 12), fg="white", bg="#1e1e2f").pack()
tk.Label(root, textvariable=memory_output, wraplength=550, fg="orange", bg="#1e1e2f", font=("Courier", 10)).pack(pady=5)

tk.Label(root, text="🛡️ Zero Trust | 🧠 Self-Evolving | 🌌 Mythic Swarm", fg="gray", bg="#1e1e2f", font=("Helvetica", 12)).pack(side="bottom", pady=10)

# 🌀 Launch Real-Time Monitor in Background
