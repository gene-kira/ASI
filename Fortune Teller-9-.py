"""
Fortune Teller — fully autonomous, enriched, and more predictive

Enhancements merged:
- Telemetry enrichment: active window focus + process tree sampling
- Persona-conditioned predictor (no manual persona selection; fully autonomous)
...
- GUI tabs restored: Predictor, Borg Mesh, Logs, Telemetry, Policy, Planner, AutoLoader, Capability
- No manual feedback UI; autonomy only
...
import subprocess, threading, time, datetime, queue, json, hashlib, math, random
...
# Optional LAN discovery (not used directly, kept for future)
try:
    from zeroconf import Zeroconf, ServiceBrowser; ZEROCONF_AVAILABLE = True
except Exception: ZEROCONF_AVAILABLE = False
try:
    from scapy.all import ARP, Ether, srp; SCAPY_AVAILABLE = True
...
# -------------------------------------------------------------------
# Telemetry collector (real probes + input + activity + personas + window focus + process trees)
# -------------------------------------------------------------------
...
class Predictor:
    def __init__(self, logger: Logger, memory: MemoryManager, policy: Policy):
        ...
        # Persona-conditioned models (fully autonomous; no manual override)
        self.persona_models = {
    ...
    def record_outcome(self, action: str, hour: int, weekday: int, success: bool, persona: Optional[str] = None):
        ...
        # Persona-conditioned histograms
...
class Planner:
    def __init__(self, logger: Logger, capability: CapabilityProfiler, dep_graph: DependencyGraph, predictor: Optional[Predictor] = None):
        ...
        self.predictor = predictor  # optional, for chain generation
    ...
    def synthesize(self, intents: List[Intent]) -> Plan:
        ...
        # Chain widening (speculative multi-step preload)
...
class GUIManager:
    ...
    def _setup_gui(self):
        ...
        self.telemetry_frame, self.telemetry_text = self._setup_tab("Telemetry", 16)
...
if __name__ == "__main__":
    print("Starting Fortune Teller (autonomous, enriched telemetry, attention, persona-conditioned, chain planning)...")
    if "--cli" in sys.argv:
