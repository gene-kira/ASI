...
class SentinelProtocol:
    ...
    def purge_expired_data(self):
        now = time.time()
        expired = []
        for k, v in self.encrypted_vault.items():
            if now - v["timestamp"] >= 86400:
                expired.append(k)
        for k in expired:
            del self.encrypted_vault[k]
            self.symbolic_memory.append({
                "event": "rebirth",
                "target": k,
                "glyph": "🌌",
...
def update_memory_display():
    logs = sentinel.symbolic_memory[-6:]
    display = "\n".join([
...
def update_node_map():
    ...
    # 🧠 Swarm Clustering Overlays
    for i, (entity, score) in enumerate(trust_map.items()):
        cluster_x = 100 if score < 0.3 else 300 if score > 0.7 else 200
        cluster_y = 100 + i * 30
        color = "red" if score < 0.3 else "green" if score > 0.7 else "yellow"
        canvas.create_oval(cluster_x-10, cluster_y-10, cluster_x+10, cluster_y+10, fill=color)
        canvas.create_text(cluster_x, cluster_y-15, text=entity, fill="white", font=("Helvetica", 8))
        canvas.create_text(cluster_x, cluster_y+15, text=f"{round(score,2)}", fill="gray", font=("Helvetica", 7))

    # 🧬 Mutation Trails Between Nodes
    low_trust = [e for e, s in trust_map.items() if s < 0.3]
    for i in range(len(low_trust)-1):
        x1, y1 = 100, 100 + i * 30
        x2, y2 = 100, 100 + (i+1) * 30
        canvas.create_line(x1, y1, x2, y2, fill="magenta", width=2, dash=(4, 2))
        canvas.create_text((x1+x2)//2, (y1+y2)//2, text="☣️", fill="magenta", font=("Helvetica", 10))
...
# 🧙 GUI Layout
root = tk.Tk()
root.title("🧙 MagicBox Defense ASI – Mythic Evolution Edition")
...
tk.Label(root, text="🛡️ Zero Trust | 🔐 Vault Rebirth | 🧬 Mutation Trails | 🧠 Swarm Clustering", fg="gray", bg="#1e1e2f", font=("Helvetica", 12)).pack(side="bottom", pady=10)
