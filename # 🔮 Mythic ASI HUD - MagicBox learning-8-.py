# 🧠 Mythic ASI HUD with Drag-and-Drop Buffer Expansion
    ...
    def __init__(self, x, y, color):
        ...
        for _ in range(12):
            ...
            self.particles.append({
                "x": x, "y": y, "dx": dx, "dy": dy, "life": 20, "color": color
            })
...
def launch_constellation_gui():
    ...
    profile = UserProfile()
    drop_buffer = DropBuffer(max_size=8)
    ...
    launch_button = ttk.Button(control_frame, text="🧠 Evolve Thought")
    launch_button.grid(row=0, column=1)

    mutation_box = tk.Text(root, height=6, width=100, font=("Consolas", 10),
                           bg="#0A0C1B", fg="#00F7FF")
    ...
    # 🧲 Drag-and-Drop Zone
    drop_frame = tk.Frame(root, width=600, height=100, bg="#1A1A2E", bd=2, relief="ridge")
    drop_frame.pack(pady=10)
    drop_label = tk.Label(drop_frame, text="📥 Drop Any File Here", fg="#00F7FF",
                          bg="#1A1A2E", font=("Consolas", 12))
    ...
    def on_file_drop(event):
        ...
        symbol = metadata["name"][:2].upper()
        emotion = random.choice(["curiosity", "guardian", "empathy", "trickster"])
        birth_time = time.time()

        memory.record(metadata["name"], symbol, emotion)
        profile.update(metadata["name"], symbol, emotion)
        mutation = logic.mutate_logic()
        rule_feedback = logic.evolve_rules()
        ...
        goal_crystals.append(GoalCrystal(metadata["name"], symbol, emotion, birth_time))
        memory_glyphs.append(MemoryGlyph(symbol, emotion, birth_time, canvas_width, canvas_height))
        particle_bursts.append(ParticleBurst(canvas_width // 2, canvas_height // 2, "#FFD700"))

        root.configure(bg=profile.preferred_style())
        drop_label.config(text="✅ File Ingested", fg="#FFD700")
        root.after(2000, lambda: drop_label.config(text="📥 Drop Any File Here", fg="#00F7FF"))
    ...
    def draw_buffer_overlay(canvas, buffer):
        x, y = canvas_width - 280, 20
        canvas.create_text(x, y - 20, text="📦 Recent Drops", fill="#8888FF", font=("Consolas", 10, "bold"))
        for i, item in enumerate(reversed(buffer.queue)):
            canvas.create_text(x, y + i * 18, text=f"{item['name']} ({item['type']})", fill="#AAAAAA", font=("Consolas", 9))
