...
import subprocess, sys, importlib, socket, threading, signal, os
...
def launch_gui():
    ...
    ttk.Label(root, text="🧠 DNS DominionDeck", font=("Consolas", 16, "bold")).pack(pady=10)
    ...
    zone_frame.pack(fill="x", padx=20, pady=10)
    domain_entry = ttk.Entry(zone_frame, width=30)
    domain_entry.pack(side="left", padx=5)
    ip_entry = ttk.Entry(zone_frame, width=20)
    ...
    trace_frame.pack(fill="both", expand=True, padx=20, pady=10)
    trace_box = tk.Text(trace_frame, bg="#1e1e1e", fg="#00ff00", font=("Consolas", 10))
    trace_box.pack(fill="both", expand=True)
    ttk.Button(root, text="Replay Trace", command=lambda: [trace_box.delete("1.0", tk.END), [trace_box.insert(tk.END, line) for line in replay_trace()]]).pack(pady=10)
