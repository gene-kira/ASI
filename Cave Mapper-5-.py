...
def guess_b4b_paths(user_name: Optional[str] = None) -> Dict[str, List[str]]:
    # Integrate Q:\ drive for Back 4 Blood; include common Steam/Epic hints
    q_drive = "Q:\\"
    ...
    # If user_name provided, include profile-based paths on Q:\Users\<User>\...
    ...
    # Also include standard %LOCALAPPDATA%/%APPDATA% paths for completeness
    ...
    items = {
        ...
        "steam_artifacts": [
            # If Steam on Q:\Program Files (x86)\Steam is used, you can add more hints here
...
DEFAULT_POLICY = {
    ...
    "auto_target_thresholds": {"cpu_pct": 12, "hit_count_min": 3, "evening_hours_bonus": 1}
}
...
class PolicyEngine:
    ...
    def auto_target_thresholds(self) -> Dict[str, Any]:
        return self.policy.get("auto_target_thresholds", {"cpu_pct": 12, "hit_count_min": 3, "evening_hours_bonus": 1})
...
class PredictiveLearner:
    ...
    - Produces confidence scores and 'why' (prefix + context)
...
# ======================================= Preloader ===================================
class Preloader:
    def __init__(self, log_q: queue.Queue, policy: PolicyEngine, user_name: Optional[str] = None):
    ...
    def targets_for(self, top: str) -> List[str]:
        ...
        if any(k in top_l for k in ["back4blood","back4blood.exe","valorant","fortnite","elden ring"]):
            # Prefer Q:\ specific paths first, then generic labels
            return [
                *(self.b4b_paths.get("game_assets", [])[:1]),
...
class AIAssist:
    """
    Guidance & auto-targeting:
    - Explains predictions (why)
    - Suggests policy tweaks based on latency/hit rates
    - Auto-targeting with classification and context weighting
    """
    def __init__(self, log_q: queue.Queue, policy: PolicyEngine):
        self.log_q = log_q; self.policy = policy
        self.hits = Counter(); self.misses = Counter()
        self.latency_obs = deque(maxlen=100)
        self.auto_candidates = Counter()

    def _log(self, msg, kind="thought"): self.log_q.put({"type": kind, "msg": msg})

    def observe_prediction(self, top: Optional[str], conf: float, why: Dict[str, Any]):
        if top:
            self._log(f"AI Assist: predicted {top} ({int(conf*100)}%), ctx={why.get('context',{})}")

    def record_outcome(self, predicted: Optional[str], actual: Optional[str], latency_ms: Optional[int] = None):
        if latency_ms is not None: self.latency_obs.append(latency_ms)
        if predicted and actual:
            if predicted == actual: self.hits[predicted] += 1
            else: self.misses[predicted] += 1

    def suggest_policy(self) -> Optional[str]:
        if not self.latency_obs: return None
        avg = sum(self.latency_obs)/len(self.latency_obs)
        ja = self.policy.jump_ahead()
        if avg > 2500 and ja < 4:
            return f"Consider increasing jump-ahead to {ja+1}s (avg launch {int(avg)}ms)."
        if avg < 1500 and ja > 1:
            return f"Consider reducing jump-ahead to {ja-1}s (avg launch {int(avg)}ms)."
        return None

    def classify(self, name: str) -> str:
        nm = name.lower()
        if any(k in nm for k in ["back4blood","game","steam","epic","anticheat","valorant","fortnite","elden ring"]):
            return "game"
        return "app"

    def auto_target(self, sysd: Dict[str, Any], focus_name: Optional[str]):
        if not focus_name: return
        thr = self.policy.auto_target_thresholds()
        cpu = sysd.get("cpu_pct") or 0
        hour = time.localtime().tm_hour
        # Context weighting: evenings boost hit counts for games
        is_evening = hour >= 18 or hour <= 1
        bonus = thr.get("evening_hours_bonus", 1) if is_evening else 0

        self.auto_candidates[focus_name.lower()] += 1 + bonus if self.classify(focus_name) == "game" else 1
        if cpu >= thr.get("cpu_pct", 12) and self.auto_candidates[focus_name.lower()] >= thr.get("hit_count_min", 3):
            aw = self.policy.always_warm()
            name = focus_name.lower()
            if name not in aw["apps"] and name not in aw["games"]:
                if self.classify(name) == "game":
                    aw["games"].append(name)
                else:
                    aw["apps"].append(name)
...
class Orchestrator(threading.Thread):
    def __init__(self, log_q: queue.Queue, ui_q: queue.Queue, policy: PolicyEngine, user_name: Optional[str] = None):
        ...
        self.preloader = Preloader(log_q, policy, user_name=user_name)
        self.decider = DecisionEngine(log_q, policy)
        self.graph = GraphStore()
        self.assist = AIAssist(log_q, policy)
        self.running = True
        ...
        while self.running:
            try:
                ...
                if focus:
                    ...
                    self.assist.auto_target(sysd, focus)
                ...
                if preds and risk["risk_low"]:
                    ...
                    self.assist.observe_prediction(top, conf, self.learner.last_why)
                    self.preloader.stage(targets, sysd)

                suggestion = self.assist.suggest_policy()
                if suggestion:
                    self._log(f"AI Assist: {suggestion}")
    ...
    def feedback_approve(self, sysd: Dict[str, Any]):
        ...
        self.assist.record_outcome(self.learner.last_top_pred, self.learner.last_top_pred, latency_ms=None)
    ...
    def feedback_deny(self, sysd: Dict[str, Any]):
        ...
        self.assist.record_outcome(self.learner.last_top_pred, None, latency_ms=None)
...
class ScannerGUI:
    def __init__(self, root: tk.Tk, inventory: InventorySnapshot, policy: PolicyEngine,
        ...
        root.title("Predictive Autonomous Scanner (Windows + Q:\\ Back 4 Blood + Auto-Targeting + AI Assist)")
        ...
        ttk.Label(pred_top, text="Top predicted targets (confidence + why + assist)").pack(anchor="w")
        ...
        self.text_why = tk.Text(self.tab_predict, height=7); self.text_why.pack(fill="x", padx=6, pady=6)
        self.text_assist = tk.Text(self.tab_predict, height=4); self.text_assist.pack(fill="x", padx=6, pady=6)
    ...
    def update_assist(self, payload: Dict[str, Any]):
        ...
        suggestion = assist.get("suggestion")
        if suggestion:
            self.text_assist.insert("end", f"Suggestion: {suggestion}\n")
...
def main():
    ...
    policy.load()
    # If your Windows username on Q:\Users\ is different, put it here (e.g., "YourName")
    # It helps the preloader include Q:\Users\<YourName>\AppData paths.
    user_name = None  # e.g., "YourName"
