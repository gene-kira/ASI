...
class MutationNode:
    def __init__(self, name, shape):
        ...
        if cuda_available:
            ...
            self.tensor = np.random.rand(*shape)

    def score_entropy(self):
        if cuda_available:
            self.entropy = torch.norm(self.tensor).item()
        else:
            self.entropy = np.linalg.norm(self.tensor)
        return self.entropy
    ...
    def mutate(self):
        ...
        if cuda_available:
            ...
            self.tensor *= factor
        print(f"[MUTATE] {self.name} mutated with factor {factor:.5f}")
