import sys, random, signal, socket, requests, ctypes
...
def run(self): 
    sock = socket.socket(socket.AF_INET,socket.SOCK_DGRAM); sock.bind(("127.0.0.1",12345))
...
def broadcast(layer,p,t): 
    try: socket.socket(socket.AF_INET,socket.SOCK_DGRAM).sendto(f"{datetime.now()}|{layer}|{p}|{t}".encode(),("127.0.0.1",12345))
