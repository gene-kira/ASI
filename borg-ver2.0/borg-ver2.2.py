...
class BorgNode:
    ...
    def scan_threat(self, sig):
        ...
        if digest[:3] == self.glyph[:3]: self.threat_level += 1; return f"🛡️ {self.id} absorbed → {digest[:12]}"
        else: self.learned.add(digest); return f"👁️ {self.id} evolved → {digest[:12]}"
    def assimilate(self):
...
class TemporalGrid:
    ...
    def update_node(self, id, stamp, field):
        if id not in self.grid_nodes: self.grid_nodes[id] = {'timestamps': [], 'field': field}
...
class OvermindUI(BoxLayout):
    ...
    def system_pulse(self, dt):
        ...
        # Cinematic visuals: particle burst + spiral orbit
        burst = ParticleBurst()
        ...
        self.add_widget(spear)
        self.add_widget(queen)
