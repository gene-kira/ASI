...
import tkinter
from asi_core import add_to_allowlist, add_to_blocklist, ALLOWLIST, BLOCKLIST
...
def launch_gui():
    ...
    root.geometry("475x400")  # Compact layout with map
    ...
    ip_list_box.grid(row=4, column=0, columnspan=3, padx=10)

    # Map Canvas
    map_canvas = tkinter.Canvas(root, width=450, height=120, bg="#1e1e2f")
    map_canvas.grid(row=5, column=0, columnspan=3, padx=10, pady=5)
