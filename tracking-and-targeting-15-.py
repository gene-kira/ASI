...
pygame.display.set_caption("Autonomous Tracking: Faces + Motion")
...
class KalmanTrack:
    def __init__(self,tid,cx,cy):
        ...
        self.label=None; self.last_bbox=None; self.time_since_update=0
...
class GreedyMultiTracker:
    def __init__(self,max_age=10,dist_threshold=70.0,max_tracks=20):
    ...
    def step(self,detections):
        ...
        for di,(cx,cy) in enumerate(det_centers):
            ...
            if best is not None and bestd<=self.dist_threshold:
                t=self.tracks[best]; t.update([cx,cy],bbox=det_bboxes[di],label=det_labels[di])
                t.history.append((int(cx*WIDTH/640),int(cy*HEIGHT/360))); used.add(best)
            else:
                if len(self.tracks)<self.max_tracks:
                    t=KalmanTrack(self.next_id,cx,cy)
                    t.update([cx,cy],bbox=det_bboxes[di],label=det_labels[di])
...
try:
    while True:
        ...
        detections_motion=detect_motion(frame)
        ...
        for t in tracks:
            ...
            label_text=f"{t.label or 'ID'} {t.id}"
            screen.blit(font.render(label_text,True,BLUE),(cx_disp+8,cy_disp-18))
