...
def launch_constellation_gui():
    ...
    # 🧲 Drag-and-Drop Zone
    drop_frame = tk.Frame(root, width=960, height=160, bg="#1A1A2E", bd=4, relief="ridge")
    drop_frame.pack(pady=10)
    drop_label = tk.Label(drop_frame, text="📥 Drop Files Up to 5GB Here", fg="#00F7FF",
                          bg="#1A1A2E", font=("Consolas", 14, "bold"))
    drop_label.place(relx=0.5, rely=0.3, anchor="center")

    # ⏳ Progress Bar
    progress_var = tk.DoubleVar()
    progress_bar = ttk.Progressbar(drop_frame, variable=progress_var, maximum=100, length=800)
  ...
  # 🔄 File Ingestion Thread
    def ingest_file(file_path):
        try:
            ...
            root.after(2000, lambda: drop_label.config(text="📥 Drop Files Up to 5GB Here", fg="#00F7FF"))
            progress_var.set(0)
