...
class MagicBoxSentinel(tk.Tk):
    def __init__(self):
        ...
        self.trust_threshold = 3  # Default kill threshold
        self.create_tabs()
    ...
    def terminate_process(self, pid, name):
        try:
            ...
            self.threat_output.insert(tk.END, msg)
            self.mutation_output.insert(tk.END, f"🧬 Mutation: {name} terminated\n")
            speak(f"{name} has been terminated. No fear.")
        except Exception as e:
            self.threat_output.insert(tk.END, f"⚠️ Failed to terminate {name}: {e}\n")
