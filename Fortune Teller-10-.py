"""
Fortune Teller — autonomous, enriched telemetry, USB + LAN-aware, and more predictive

Enhancements:
- Telemetry enrichment: active window focus + process tree sampling
- Persona-conditioned predictor (fully autonomous; no manual selection)
- Transformer-lite attention bias for longer sequence context
- Adaptive exploration based on anomaly and entropy
- Multi-step predictive planning chains
- Confidence-weighted autonomy gating
- Capability-aware prefetch scaling
- USB drive detection via pyudev (Linux) and psutil (Windows/macOS fallback)
- Local network share detection via psutil disk partitions (SMB/CIFS/NFS/UNC)
- Optional LAN scan via Scapy ARP and Zeroconf service browsing
- GUI tabs: Predictor, Borg Mesh, Logs, Telemetry, Policy, Planner, AutoLoader, Capability
- Elevation-safe, encrypted logging/memory, Borg Mesh overlay
"""
...
# Optional Linux USB via udev
try:
    import pyudev; PYUDEV_AVAILABLE = True
except Exception: PYUDEV_AVAILABLE = False
...
# -------------------------------------------------------------------
# Telemetry collector (now with USB + network shares + LAN scan)
# -------------------------------------------------------------------
...
class TelemetryCollector:
    ...
    def _usb_snapshot(self, max_items: int = 20) -> List[Dict[str, Any]]:
        devices: List[Dict[str, Any]] = []
        try:
            if PYUDEV_AVAILABLE and platform.system() == "Linux":
                ctx = pyudev.Context()
                for dev in ctx.list_devices(subsystem='usb'):
                    info = {
                        "device": dev.device_path,
                        "vid": dev.attributes.get("idVendor").decode() if dev.attributes.get("idVendor") else None,
                        "pid": dev.attributes.get("idProduct").decode() if dev.attributes.get("idProduct") else None,
                        "serial": dev.attributes.get("serial").decode() if dev.attributes.get("serial") else None,
                    }
                    devices.append(info)
                    if len(devices) >= max_items:
                        break
            else:
                # Fallback: detect removable/mounted drives via psutil
                for part in psutil.disk_partitions(all=False):
                    # Windows: 'removable' is not directly flagged; we estimate via device path or fs type
                    fs = (part.fstype or "").lower()
                    mp = part.mountpoint
                    dev = part.device
                    is_removable = ("removable" in dev.lower()) or ("media" in mp.lower())
                    if platform.system() == "Windows":
                        # If drive letter present and not fixed type, approximate
                        is_removable = is_removable or (dev.upper().startswith("\\\\?\\") and "usb" in dev.lower())
                    if is_removable:
                        devices.append({"device": dev, "mountpoint": mp, "fstype": fs})
                        if len(devices) >= max_items:
                            break
        except Exception as e:
            self.logger.log("telemetry", "USB snapshot failed", {"error": str(e)})
        return devices
    ...
    def _lan_scan(self, timeout: float = 1.5, max_hosts: int = 32) -> Dict[str, Any]:
        ...
        try:
            ...
            if SCAPY_AVAILABLE and cidr:
                # Limit scan to a subset for speed
    ...
    def snapshot(self) -> Dict[str, Any]:
        ...
        usb_devices = self._usb_snapshot()
...
# -------------------------------------------------------------------
# Predictor (ensemble + personas + k-gram + bandit + calibration + attention)
# -------------------------------------------------------------------
...
class Predictor:
    ...
    def predict_with_temp(self, snapshot: Dict[str, Any], extra_temperature: float = 0.0) -> List[Intent]:
        ...
        for action, base_w in self.action_weights.items():
            ...
            score = (
                s_base + s_time + s_markov + s_kgram + s_input + s_ctx + s_feedback +
                s_novelty + s_session + s_persona + s_rhythm + s_cooldown + s_piacc + s_bandit + s_attn
            )
        ...
        base_temp = self._adaptive_temperature(base_temp=0.8)
        temp = max(0.35, min(1.4, base_temp + extra_temperature))
...
class Planner:
    ...
    def _predict_chain(self, intent: Intent, max_len: int = 3) -> List[str]:
        chain = [intent.id]
        last = intent.id
    ...
    def synthesize(self, intents: List[Intent]) -> Plan:
        ...
        chain = self._predict_chain(top, max_len=3)
        for act in chain[1:]:
...
class GUIManager:
    def __init__(self, predictor: Predictor, telemetry: TelemetryCollector,
        ...
        if TK_AVAILABLE:
            self.root = tk.Tk()
            self.root.title("Fortune Teller — Autonomous")
...
if __name__ == "__main__":
    print("Starting Fortune Teller (autonomous, enriched telemetry incl. USB/LAN, attention, persona-conditioned, chain planning)...")
    if "--cli" in sys.argv:
