...
DEFAULT_POLICY = {
    ...
    "auto_target_thresholds": {"cpu_pct": 10, "hit_count_min": 3}
}
...
class PolicyEngine:
    ...
    def auto_target_thresholds(self) -> Dict[str, Any]:
        return self.policy.get("auto_target_thresholds", {"cpu_pct": 10, "hit_count_min": 3})
...
class AIAssist:
    ...
    - Suggests policy tweaks based on observed latency and hit/miss rates
    - Auto-targeting: promotes frequent high-CPU processes into always_warm
    """
    def __init__(self, log_q: queue.Queue, policy: PolicyEngine):
        self.log_q = log_q; self.policy = policy
    ...
    def auto_target(self, sysd: Dict[str, Any], focus_name: Optional[str]):
        ...
        cpu = sysd.get("cpu_pct") or 0
        if cpu >= thr.get("cpu_pct", 10):
            self.auto_candidates[focus_name.lower()] += 1
            if self.auto_candidates[focus_name.lower()] >= thr.get("hit_count_min", 3):
                aw = self.policy.always_warm()
                name = focus_name.lower()
                if name not in aw["apps"] and name not in aw["games"]:
                    # Promote to games if looks like a game, else app
                    if any(k in name for k in ["back4blood","game","steam","valorant","fortnite","elden ring"]):
                        aw["games"].append(name)
                    else:
                        aw["apps"].append(name)
                    self.policy.policy["always_warm"] = aw
                    self._log(f"AI Assist: auto-targeted '{name}' into always_warm.")
                    # Save silently (fallback aware)
...
class ScannerGUI:
    def __init__(self, root: tk.Tk, inventory: InventorySnapshot, policy: PolicyEngine,
        ...
        root.title("Predictive Autonomous Scanner (Windows + Back 4 Blood + Auto-Targeting + AI Assist)")
