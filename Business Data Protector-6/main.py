...
# ✅ GUI engine fallback
def ensure_gui_engine():
    try:
        from hidpi_tk import DPIAwareTk
        return DPIAwareTk
    except ImportError:
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "hidpi-tk"])
            from hidpi_tk import DPIAwareTk
            return DPIAwareTk
        except Exception as e:
            print(f"⚠️ hidpi-tk failed: {e}")
            from tkinter import Tk
            return Tk

# ✅ Auto-install required libraries
required_libs = ["tkinter", "psutil", "requests", "sklearn", "numpy"]
...
def launch():
    ...
    TkEngine = ensure_gui_engine()
    app = MythicNodeGUI(TkEngine)
    brain = ASIBrain(app.memory)
    brain.train_model([[20, 30], [50, 60], [100, 200], [10, 15]])
