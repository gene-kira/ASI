...
# Wired to Windows 11 XInput controllers via the 'inputs' library.
...
# ========= Gamepad polling =========
def poll_input_mag():
    """Poll XInput controller and return normalized magnitude (0–1)."""
    if not get_gamepad:
        return 0.0
    try:
        events = get_gamepad()
        mag = 0.0
        for e in events:
            if e.code in ("ABS_X", "ABS_Y"):  # left stick axes
...
class CodexGUI(tk.Tk):
    def __init__(self, swarm: Swarm):
        super().__init__()
        self.title("Codex Predictive Frame Monitor (XInput)")
    ...
    def loop(self):
        ...
        # Input magnitude from XInput
        input_mag = poll_input_mag()
        ...
        # Swarm ops
        if self.step_i % 20 == 0: self.swarm.broadcast()
        if self.step_i % 40 == 0: self.swarm.consensus_round()

        # Redraw charts
        self._draw_error(); self._draw_mutation(); self._draw_swarm_heatmap()
        self.step_i += 1
    ...
    def _draw_error(self):
        self.ax_err.clear(); self.ax_err.set_title("Prediction Error Over Time")
        ...
        self.ax_err.set_ylabel("Error"); self.ax_err.grid(True, alpha=0.3)
