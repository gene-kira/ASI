...
def simulate_swarm_sync():
    messagebox.showinfo("Action", "Swarm sync simulated. Nodes merged purge rules and threat signatures.")

# 🧠 Threat Matrix Scan
def scan_threat_matrix():
    messagebox.showinfo("Action", "Threat matrix scanned. Resurrection glyphs detected and logged.")
...
def remove_media_player():
    cmd = 'powershell -Command "Disable-WindowsOptionalFeature -Online -FeatureName WindowsMediaPlayer -NoRestart"'
    subprocess.call(cmd, shell=True)
    messagebox.showinfo("Removed", "Windows Media Player has been disabled.")

def disable_defender():
    warning = messagebox.askyesno("Warning", "Disabling Defender is risky. Are you installing another antivirus?")
    if warning:
        cmd = 'powershell -Command "Set-MpPreference -DisableRealtimeMonitoring $true"'
        subprocess.call(cmd, shell=True)
        messagebox.showinfo("Defender", "Microsoft Defender real-time protection disabled.")

# 🖼️ GUI Layout
def build_gui():
    root = tk.Tk()
    root.title("Codex Purge Shell: Sovereign Edition")
    ...
    panels = {
        ...
        "Component Manager": [
            ("Open Add/Remove Programs", open_add_remove_programs),
            ("Open Optional Features", open_optional_features),
            ("Remove Windows Media Player", remove_media_player),
            ("Disable Microsoft Defender", disable_defender)
        ]
    }
